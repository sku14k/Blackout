...
(function($, ATVI) {
        ...
        for(var i = 0; i < robjs.length; i++) {
            ...
            list.onChange(function(ev) {
                ...
                */
                ...
                where.updateAnalyticsObject(context);
                if (ATVI.analytics) where.sendStatusEvent(context, ev);
            });
        }
    };
    ...
    */
    ...
    var buildRetailerList = function (context, rows) {
        ...
        if (elems.length && v != elems[0].id) v = elems[0].id;
    ...
    lb.selectbox = function (context, elems, wrapper, field) {
        ...
        ret.buildFromElems = function() {
            ...
            if(elems.length == 0) {
                ...
                for(var i = 0; i < elems.length; i++) {
                    $("<option>", {class: "possible field-" + elems[i].id, value: elems[i].id}).html(elems[i].text).appendTo($selectWrapper);
                }
            }
        };
...
///////////////////////////////////
            ...
            where.onSelectboxUpdate = function(context, $el, listObj) {
                ...
                var $bundleEls = context.wrapper.find('.bundle-content-container'),
                    $platformEls = context.wrapper.find('.platform-content-container'),
                    bundleId = context.status.bundles,
                    platformId = context.status.platforms;
                if (bundleId) {
                    wtb.filterContentByType($bundleEls, 'bundle', bundleId);
                }
                if (platformId) {
                    wtb.filterContentByType($platformEls, 'platform', platformId);
                }
    ...
    wtb.filterContentByType = function($els, type, id) {
		var $parents = $els.parent();
        if (id == "") id = 'default';
        // find any parents with no matching child >>> show the 'default' child
        $parents.each(function() {
			var thisParent = $(this);
            if (!thisParent.children('[data-' + type + '-content-id=' + id + ']').length) {
                thisParent.children().hide().filter(function() {
					return $(this).data(type + '-content-id') == 'default';
                }).show();
            } else {
				thisParent.children().hide().filter(function() {
            		return $(this).data(type + '-content-id') == id;
        		}).show();
            }
        });
    };
    ...
    var sortFields = function(context) {
        ...
        var bundleOrder = ["x", "digital-deluxe", "digital-deluxe-enhanced", "collectors-edition", "pro-edition", "digital-standard", "standard"];
        ...
        $.each(obj, function() {
            ...
            if (fieldName === 'retailers' && len === 1) {
                var url = context.listsObj.retailers[0].elems[0].id,
                    selectWrapper = context.wrapper.find('.retailers-entry .selectricWrapper');
